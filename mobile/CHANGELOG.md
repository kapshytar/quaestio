...
- **Fix (Android): WebView missing in every tab after the OS kills the app in background and the Activity is restored from `savedInstanceState`** — `MainActivity.loadSlotWithSessionOverride` looked up the restored tab only via `ChatPagerAdapter`'s in-memory map, which `FragmentStateAdapter` never repopulates when it restores fragments directly from the `FragmentManager` after process death; it now falls back to `findFragmentByTag("f$slotIndex")`. That fallback still raced `FragmentStateAdapter`, which only re-adds a restored fragment to the `FragmentManager` when ViewPager2 actually binds its page (`placeFragmentInViewHolder`) — later than all 8 retries (~2s) for every slot, and arbitrarily later for background/offscreen slots — so every tab stayed a black screen. Inverted to a pull model: `ChatFragment.onViewCreated` now calls `MainActivity.onChatFragmentViewReady(slotIndex, this)` as soon as its own view (and `slotIndex` from `arguments`) exist; `MainActivity` tracks slots with an unfinished load in `pendingSlotLoads` (added when `loadSlotWithSessionOverride` starts, removed once it actually finds a fragment) and, if the slot is still pending when its view becomes ready, loads it immediately instead of waiting on the next retry tick or the ViewPager2 bind.

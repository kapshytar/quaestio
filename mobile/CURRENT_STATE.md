...
- 2026-07-15 (2.9.7): iOS and Android perform at most one background startup session reconcile when the last successful web refresh is 15+ minutes old, preserve local-only rows, leave the active session context untouched, and display the persisted web-sync time in the Sessions list.
- 2026-07-15 (2.9.6): iOS and Android ingest snapshots persist the backend-returned effective project tag; a continuation displays under the project inherited from its session chain instead of a stale picker selection.
- 2026-07-15 (2.9.5): project tree collapsed by default on all clients, expansion persisted (iOS UserDefaults / Android SharedPreferences / desktop localStorage, desktop 1.118.0); iOS main picker consolidated onto the filter tree's `visibleFilterTreeRows` walk (duplicate `flattenProjects` removed).

...
- 2026-07-15 (1.118.2): signed-in startup performs one background web-session reconcile only when the last refresh is at least 15 minutes old; it refreshes the saved index without rebinding the active session, and the Sessions list displays the persisted last web-sync time.
- 2026-07-15 (1.118.1): ingest snapshots persist the backend-returned effective project tag, so a continuation remains displayed under its inherited session project even when the picker currently points elsewhere.
- 2026-07-15 (1.118.0): project trees (main panel + sessions filter) collapsed by default, expansion persisted in localStorage per tree.
- 2026-07-15 (1.117.0): stale-context self-heal for Collect/Merge (`repairStaleQuestionContext`: note→session lookup via new `dream-get-note-session-id` IPC, tail re-point, fail-closed on network, one-shot P0001 retry, merge-bootstrap suppressed on unrepaired context); sessions-list cache freshness stamped only by full list fetches (partial save/delete invalidate) — fixes session project not restoring on load; Collect single-flight.
- 2026-07-15 (1.116.0): session identity now resolves via per-slot conversation-key matching against saved snapshots (`restoreStoredQuestionContextForCurrentSlots`, iOS-parity) as a fallback to the legacy exact-fingerprint global context; fixes duplicate-session minting on enabled-slot change / relaunch (S304). Known parity gaps still open (from the 2026-07-15 Codex audit): pending-navigation target not substituted on desktop; tail-refresh at restore; Grok `/project/<id>?chat=<id>` identity collapses to project id on ALL clients. Continuation project inheritance is now backend-owned: the current UI project is ignored and clients persist the effective project returned by ingest.
